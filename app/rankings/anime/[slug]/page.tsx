...
export async function generateStaticParams() {
  const rankings = await getRankings("anime", 100);
...
export default async function AnimeMoodListPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const ranking = await getRankingBySlug("anime", slug);
  if (!ranking) notFound();
  ...
  const breadcrumbLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Anime rankings", item: `${BASE_URL}/rankings/anime` },
  ...
  return (
    <div className="mx-auto max-w-5xl px-6 py-16">
      ...
      <nav className="text-xs text-marquee-textDim" aria-label="Breadcrumb">
        ...
        {" / "}
        <Link href="/rankings/anime" className="hover:text-marquee-gold">Anime</Link>
        {" / "}
      ...
      <div className="mt-10 grid gap-4 sm:grid-cols-2">
        {ranking.items.map((item, index) => (
          <Link
            ...
            href={`/anime/search?q=${encodeURIComponent(item.title)}`}
      ...
      <div className="mt-10 flex flex-wrap gap-4">
        <Link href="/rankings/anime" className="rounded border border-marquee-line px-4 py-2 text-marquee-text hover:border-marquee-gold">
          More anime rankings
        </Link>
        <Link href="/anime" className="rounded border border-marquee-line px-4 py-2 text-marquee-text hover:border-marquee-gold">
          Anime hub
        </Link>
      </div>
    </div>
