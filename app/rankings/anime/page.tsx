...
export default async function AnimeRankingsPage({ searchParams }: AnimeRankingsPageProps) {
  ...
  const moodLists = await getRankings("anime", 24);
  const totalPages = 5;

  return (
    <div className="mx-auto max-w-6xl px-6 py-16">
      <p className="font-mono text-xs tracking-[0.3em] text-marquee-gold">⭐ anime</p>
      <h1 className="mt-3 font-display text-3xl sm:text-5xl text-marquee-text">Anime rankings</h1>
      <p className="mt-3 max-w-2xl text-marquee-textDim">
        Curated mood and decade lists, plus a permanent top-100 ranking view powered by Jikan data.
      </p>
      ...
      {moodLists.length > 0 && (
        <>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {moodLists.map((list) => (
              <Link key={list.slug} href={`/rankings/anime/${list.slug}`} className="ticket p-5 hover:border-marquee-gold transition-colors">
